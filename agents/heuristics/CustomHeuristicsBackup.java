...
public class CustomHeuristicsBackup
{
...
public static double pawnStructure(DFSTreeNode node){
	    ...
	    for (Piece piece : node.getGame().getBoard().getPieces(node.getGame().getCurrentPlayer())) {
	        if (piece.getType() == PieceType.PAWN) {
	            ...
	            // Check for supported pawns
	            for (Piece otherPiece : node.getGame().getBoard().getPieces(node.getGame().getCurrentPlayer())) {
	                if (otherPiece.getType() == PieceType.PAWN && otherPiece != piece) {
	                    Coordinate otherPawnPosition = node.getGame().getCurrentPosition(otherPiece);
	                    int rowOffset = node.getGame().getCurrentPlayer().getPlayerType() == PlayerType.WHITE ? -1 : 1;
	                    if (otherPawnPosition.getXPosition() == pawnColumn && otherPawnPosition.getYPosition() == pawnRow + rowOffset) {
	                        supportedPawns++;
	                        break;
	                    }
	                }
	    ...
	    double pawnStructureScore = -0.5 * doubledPawns - 0.75 * isolatedPawns + passedPawns + 0.5 * supportedPawns;
...
//		System.out.println("The number of pieces we are threatening are " + numPiecesWeAreThreatening);
	...
	public static double Development(DFSTreeNode node) {
	    ...
	    int[] centralColumns = {4, 5};
	    int[] centralRowsBlack = {5};
	    int[] centralRowsWhite = {4};
...
//        System.out.println("Enemy has " + enemySumOfMaterial);
        ...
        return sumOfMaterial - enemySumOfMaterial; // this is supposed to give opponents material, but gives ours
    }
...
//		System.out.println("The score to Move Away from Initial Position is = " + val);
	...
	public static double getHeuristicValue(DFSTreeNode node)
	{
		...
		double InitialMoveAway = moveAwayInit(node);
		...
		// please replace this!
		return 10*offenseHeuristicValue + defenseHeuristicValue + nonlinearHeuristicValue + 2*materialValue + pawnStructureValue + InitialMoveAway + developmentValue;
	}
